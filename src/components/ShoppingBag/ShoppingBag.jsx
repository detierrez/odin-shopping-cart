import c from "./ShoppingBag.module.css";

export default function ShoppingBag({ count = 0 }) {
  let countElement;
  if (count > 0) {
    let countClass = c.count;
    let countContent = count;
    if (count > 99) {
      countClass += " " + c.tooMany;
      countContent = "99+";
    }

    countElement = <span className={countClass}>{countContent}</span>;
  }
  return (
    <div className={c.bag}>
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        <title>Go to cart</title>
        <path d="M12,13A5,5 0 0,1 7,8H9A3,3 0 0,0 12,11A3,3 0 0,0 15,8H17A5,5 0 0,1 12,13M12,3A3,3 0 0,1 15,6H9A3,3 0 0,1 12,3M19,6H17A5,5 0 0,0 12,1A5,5 0 0,0 7,6H5C3.89,6 3,6.89 3,8V20A2,2 0 0,0 5,22H19A2,2 0 0,0 21,20V8C21,6.89 20.1,6 19,6Z" />
      </svg>
      {countElement}
    </div>
  );
}
